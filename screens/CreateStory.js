...
export default class CreateStory extends Component {
  ...
  addstory =  async()=>{
    if(this.state.title && this.state.description && this.state.story && this.state.moral){
      ...
      await firebase.database().ref("/posts/"+Math.random().toString(36).slice(2)).set(storydata).then(function(snapshot){});
      this.props.setUpdateToTrue();
      this.props.navigation.navigate('Feed')
    }else{
      Alert.alert("error","all fields are require",[{text:'ok',onPress:()=>console.log("ok pressed")}],{cancelable: false})
    }
  }
  render() {
    if (!this.state.fontsLoaded) {
      ...
      return (
        <View style={this.state.light_theme?styles.containerLight:style.container}>
            ...
            <ScrollView>
              ...
              <View style={{ height: RFValue(this.state.dropdownHeight) }}>
                <DropDownPicker
                  ...
                  dropDownStyle={this.state.light_theme?styles.ddLight: styles.dd  }
                  labelStyle={{
                    color: "white",
